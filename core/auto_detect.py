...
_UNIT_ABBREVS = (
    ...
    'ЭЛОУ', 'АВТ', 'ЭЛОУ-АВТ', 'ЭЛОУ-АТ',
    # Крекинг
    'УКК', 'УКТКК', 'ФКК', 'ККФ', 'ККМС',
    # Гидрокрекинг / гидроочистка
    'УГК', 'УГБК', 'УПБК', 'УГО', 'УГОДТ', 'УГОБ', 'УГОК', 'УГОМ', 'УГОНК',
    'ГО', 'ГОД', 'ГОБ', 'ГОБКК', 'ГОК', 'ГОВС', 'ГОМС',
    # Риформинг / изомеризация
    'УКР', 'УКРН', 'ККР', 'УИ', 'УИП', 'УИЛФ',
    # Алкилирование
    'УА', 'УСАА', 'УФКА',
    # Коксование
    'УЗК', 'УЗПК', 'УПК',
...
RE_REFINERY_UNIT = re.compile(
    ...
    r'|ЛЧ[-\s]*\d{1,2}[-\s]*/?\d{2,4}'                                 # ЛЧ-24-2000
    r'|ЛГ[-\s]*\d{1,2}[-\s]*/?\d{2,4}(?:[-/]\d+)?'                     # ЛГ-35-11/300
    r'|Л[-\s]*\d{1,2}[-\s]*/?\d{2,4}(?:[-/]\d+)?'                      # Л-24-7
    r'|Г[-\s]*\d{1,2}[-\s]*/?\d{2,4}[А-Яа-яМ]*'                       # Г-43-107М
    r'|\d{2}-\d{2,3}(?:/\d+[А-Яа-я]*)?',                               # 43-102, 21-10/3М
    re.IGNORECASE,
)
