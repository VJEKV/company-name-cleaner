...
# Шифры установок НПЗ и нефтехимии
# Числовые коды: ЛЧ-24-2000, ЭЛОУ-АВТ-6, Л-35-11/300, Г-43-107М
# Аббревиатуры: УКК, УГБК, ГФУ, УЗК, МТБЭ и т.д.
# Только аббревиатуры 3+ букв (двухбуквенные ГО, КТ, ВБ, УА и т.д. дают
# тысячи ложных срабатываний внутри обычных слов)
_UNIT_ABBREVS = (
    # Первичная переработка
    'ЭЛОУ-АВТ', 'ЭЛОУ-АТ', 'ЭЛОУ', 'АВТ',
    # Крекинг
    'УКК', 'УКТКК', 'ФКК', 'ККФ', 'ККМС',
    # Гидрокрекинг / гидроочистка
    'УГК', 'УГБК', 'УПБК', 'УГОДТ', 'УГОБ', 'УГОК', 'УГОМ', 'УГОНК',
    'ГОБКК', 'ГОВС', 'ГОМС',
    # Риформинг / изомеризация
    'УКР', 'УКРН', 'ККР', 'УИП', 'УИЛФ',
    # Алкилирование
    'УСАА', 'УФКА',
    # Коксование
    'УЗК', 'УЗПК', 'УПК',
    # Газопереработка
    'ГФУ', 'ЦГФУ', 'АГФУ', 'УПГ', 'УОПГ', 'УКПГ',
    # Сера / очистка
    'УПС', 'УРС', 'УАО',
    # Висбрекинг / термокрекинг
    'УВБ', 'УТК',
    # Битум / масла
    'УПБ', 'УСОМ', 'УДАМ', 'УДАП', 'УДПМ', 'УГДП', 'УГИМ',
    # Водород
    'УПВ', 'УКПВ', 'ПСА',
    # МТБЭ / оксигенаты
    'МТБЭ', 'ТАМЭ', 'УМТБЭ', 'УТАМЭ',
    # Нефтехимия / пиролиз
    'УПНС', 'УПУГ', 'УПЭ', 'УПП',
    'УППЭ', 'УПВД', 'УПНД', 'УППП', 'УПММ',
    'УДГ', 'УДГИБ', 'УДГПР', 'УБТЭ', 'УЭБ', 'УКАБ',
    # Смешение
    'УСТ', 'УСМТ',
    # Комбинированные
    'КГК', 'КГО',
)
_UNIT_ABBREV_PAT = '|'.join(re.escape(a) for a in sorted(_UNIT_ABBREVS, key=len, reverse=True))

RE_REFINERY_UNIT = re.compile(
    r'(?<![А-Яа-яЁё])'                                                  # НЕ внутри слова
    r'(?:(?:' + _UNIT_ABBREV_PAT + r')[-\s]*\d*(?:[-/]\d+[А-Яа-яМу]*)*)' # аббревиатура + числа
    r'(?![А-Яа-яЁё])'                                                    # НЕ продолжение слова
    r'|(?<![А-Яа-яЁё])ЛЧ[-\s]*\d{1,2}[-\s]*/?\d{2,4}'                  # ЛЧ-24-2000
    r'|(?<![А-Яа-яЁё])ЛГ[-\s]*\d{1,2}[-\s]*/?\d{2,4}(?:[-/]\d+)?'      # ЛГ-35-11/300
    r'|(?<![А-Яа-яЁё])ЛК[-\s]*\d+[а-яу]*(?![А-Яа-яЁё])'               # ЛК-6у
    r'|(?<![А-Яа-яЁё])ЭП[-\s]*\d{2,3}(?![А-Яа-яЁё])'                  # ЭП-300
    r'|(?<![А-Яа-яЁё])КТ[-\s]*\d+(?:/\d+)?(?![А-Яа-яЁё])',             # КТ-1
    re.IGNORECASE,
)
...
# Сокращения требуют точку (ул., пр., пер., наб., пл.) или полное слово
RE_ADDRESS_SHORT = re.compile(
    r'(?<![а-яА-ЯёЁ])'  # НЕ после буквы (исключает "руб.", "особ." и т.д.)
    r'(?:ул(?:ица|\.)[-\s]+|пр(?:оспект|\.)[-\s]+|пер(?:еулок|\.)[-\s]+|'
    r'(?:б-р|бульвар)[-.\s]+|шоссе[-\s]+|наб(?:ережная|\.)[-\s]+|'
    r'пл(?:ощадь|\.)[-\s]+|мкр[-.\s]*н?[-.\s]+|проезд[-\s]+)'
    r'[А-ЯЁ][А-ЯЁа-яё0-9\s./-]{2,40}?'
    r',\s*\d+[а-яА-Я/]*'
    r'(?:,?\s*(?:корп|стр|к)[-.\s]*\d+[а-яА-Я]*)?'
    r'(?:,?\s*(?:кв|оф|офис|пом|каб|комн|этаж)[-.\s]*\d+)?',
    re.IGNORECASE,
...
RE_ADDRESS_FULL = re.compile(
    ...
    r'(?:(?:ул(?:ица|\.)|пр(?:оспект|\.)|пер(?:еулок|\.)|б-р|бульвар|шоссе|наб(?:ережная|\.)|пл(?:ощадь|\.)|проезд|мкр)[-\s]+[А-ЯЁа-яё0-9][-А-ЯЁа-яё0-9\s./]+?'
    r'(?:,?\s*(?:д(?:ом)?[-.\s]*)?)\d+[а-яА-Я/]*)?'
    r'(?:,?\s*(?:корп|стр|к)[-.\s]*\d+)?'
    r'(?:,?\s*(?:кв|оф|офис|пом|каб|комн|этаж)[-.\s]*\d+)?',
    re.IGNORECASE,
...
# ── Встроенные исключения (ложные срабатывания) ──

_BUILTIN_EXCLUSIONS = frozenset({
    # Бизнес-термины (с заглавной в контрактах)
    'подрядчика', 'подрядчику', 'подрядчиком', 'подрядчик',
    'заказчика', 'заказчику', 'заказчиком', 'заказчик',
    'субподрядчика', 'субподрядчику', 'субподрядчиком', 'субподрядчик',
    'субподрядчиков', 'субподрядчиками',
    'генподрядчик', 'генподрядчика', 'генподрядчиков',
    'исполнитель', 'исполнителя', 'покупатель', 'покупателя',
    'оператор', 'оператора', 'планировщик',
    'куратор', 'куратора',
    # Роли / должности
    'генерального директора', 'главного инженера', 'главного механика',
    'техническому', 'директор', 'директора',
    # Аббревиатуры
    'окр', 'тр', 'торо', 'сро', 'гк', 'от', 'гу', 'нк', 'пб', 'лна',
    'ису', 'сиз', 'акз', 'рвс', 'лкм', 'лкп', 'сап', 'еи',
    'гпу', 'ор', 'сто', 'гсо', 'итр', 'дебл', 'ксг', 'эдо', 'эд',
    'окн', 'огм', 'огэ', 'укб', 'окб', 'пс', 'гпу-1',
    'опбиот', 'опбиот-4', 'опбиот-7', 'опбиот-11', 'соуэ',
    'отн', 'отн-35', 'огэ-1', 'огэ-18', 'кс', 'окпо',
    'егрюл', 'мот', 'оон',
    # Обычные слова (ложно детектируются как фамилии)
    'ценовых', 'заказов', 'свободного', 'вызов', 'тару',
    'величина', 'членов', 'журнал', 'журнале', 'справке',
    'протоколе', 'протоколом', 'графиками', 'интернет',
    'конфиденциальность', 'сторон', 'акту', 'споров',
    'допустил', 'сдал', 'согласовано', 'несчастный',
    'распитие', 'несанкционированное', 'несанкционированные',
    'подоперация', 'автокран', 'изол', 'демонт', 'огнезащ', 'осн',
    'общества', 'общество',
    # Организационные термины
    'нефтехим', 'госстроя', 'ростехнадзора', 'ростехнадзору', 'ростехнадзоре',
    'росфинмониторинга', 'госпожнадзор', 'минстроя',
    'сбербанка', 'банка россии',
    'oracle', 'sap',
    # Документные шаблоны
    'ф.и.о.', 'ф.и.о', 'ф.и', 'и.о. фамилия', 'м.п.', 'м.п',
    'юридический адрес',
    # Длинные фразы (ложно детектируются как адреса)
    'и фактический адреса, для последующего проведения аудита',
    'заказчика, указанный в разделе 18 данного договора',
})
...
def _filter_exclusions(entities: list[DetectedEntity]) -> list[DetectedEntity]:
    ...
    if _exclusions_cache is None:
        try:
            ...
            _exclusions_cache = _BUILTIN_EXCLUSIONS | UserDictionary.get_exclusions()
        except Exception:
            _exclusions_cache = _BUILTIN_EXCLUSIONS
